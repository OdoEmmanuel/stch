import React from 'react';

interface CheckMarkButtonProps {
  width?: number;
  height?: number;
  className?: string;
}

const CheckMarkButton: React.FC<CheckMarkButtonProps> = ({ 
  width = 22, 
  height = 21, 
  className = '' 
}) => {
  return (
    <svg
      width={width}
      height={height}
      viewBox="0 0 22 21"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      className={className}
    >
      <rect width="22" height="21" fill="url(#pattern0_395_1250)" />
      <defs>
        <pattern
          id="pattern0_395_1250"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use
            xlinkHref="#image0_395_1250"
            transform="matrix(0.0138889 0 0 0.0145503 0 -0.0238095)"
          />
        </pattern>
        <image
          id="image0_395_1250"
          width="72"
          height="72"
          preserveAspectRatio="none"
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </svg>
  );
};

export default CheckMarkButton;